 ...
 *
 * @wordpress-plugin
 * Plugin Name:       Facebook Page Stats
 ...
 * Text Domain:       facebook-page-stats
 ...
 */
require plugin_dir_path( __FILE__ ) . 'includes/fps.php';
...
include_once plugin_dir_path( __FILE__ ) . 'admin/partials/fps-admin-display.php';
function fps_settings() {
    add_menu_page('FB Page Stats', 'FB Page Stats', 'administrator', 'fps_settings', 'fps_display_settings',  'dashicons-facebook');
}
add_action('admin_menu', 'fps_settings');
...
add_shortcode('shares', 'fps_shares');


// comments shortcode
function fps_comments() {
	global $shortcode_pageURL;
	$counts_array = dataCheck($shortcode_pageURL);
	echo '<span class="fb-comments">'.$counts_array[2][1].'</span>';
}
add_shortcode('comments', 'fps_comments');